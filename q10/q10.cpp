...
int insertConsonants(char input_characters[], int num_elements, int ARR_SIZE, int num_consonants)
{
    ...
    else
    {
        ...
        {
            if(input_characters[i] != 'a' && input_characters[i] != 'e' && input_characters[i] != 'i' && input_characters[i] != 'o' && input_characters[i] != 'u')
            {
                char char_to_insert = input_characters[i] - 32;
                insertAfter(input_characters, num_elements, ARR_SIZE, i, char_to_insert);
            }
        }
        return num_elements + num_consonants;
    }
}
